...
Make targeted edits to an existing spec's `spec.md`, `plan.md`, or `tasks.md` without re-running `/sf:spec`. Status changes trigger a roadmap refresh.
...
6. If any frontmatter `status:` field changed (e.g. `planning` -> `implementing`, `implementing` -> `shipped`), run `/sf:refresh-roadmap` to rewrite `ROADMAP.md`.
