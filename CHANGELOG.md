...
### Fixed (second adversarial verification round over the fixes themselves)
- `sf-doctor.sh` died mid-report (false exit 1) on any project without `.claude/` — the pre-adoption first-contact flow — and on projects with an unreadable subdir; both find-pipelines made failure-tolerant.
- The coexistence warning (now on stdout, i.e. injected into Claude's context) fired in **non-adopted** projects that have their own `session-start.sh` hook, advising an `/sf:adopt` that would have detached the user's hook; now gated on adoption + the hook actually being the savvy engine copy.
- The engine-version stamp marked **legacy in-tree** projects as plugin-adopted whenever the plugin was installed at user scope, permanently locking them out of `/sf:upgrade`; the stamp now requires `sf@savvy` enabled at project scope, and the upgrade skill's guard ignores a stray stamp when the in-tree engine is still present.
- `v1.4.0.sh` fetched baselines from the immutable `v1.4.0` tag — which cannot contain the three baselines generated after it was cut; baselines now fetch from `v1.5.0`.
- The sticky-conflict marker could be cleared by "keep my version", recreating the second-upgrade silent overwrite; clearing now requires the on-disk file to equal the target, everywhere the rule is stated.
- The legacy `template/` mirror no longer ships `adopt`/`doctor`/`eject` commands or the `project-adopt` skill (they invoke `${CLAUDE_PLUGIN_ROOT}/scripts/*`, which does not exist in-tree).
- README no longer overclaims "hooks only act in adopted projects" (secret-scan is a deliberate everywhere-guard); CI shellcheck now also covers `install.sh` and the template hooks; the macOS CI job pins the test-body interpreter (a PATH shim prevents a silent bash-5 hijack of the "system bash 3.2" gate); `release.yml` compares manifests date-normalized instead of exempting them, fails on untracked build output, and refuses empty release notes; `tests/run.sh` requires a git checkout with a clear message.
