...
7b. If this repo is the framework itself (a `VERSION` file and `scripts/gen-manifest.sh` exist at root), write the confirmed version into `VERSION`, then run `scripts/build-plugin.sh` (which runs `gen-manifest.sh`, stamps `plugin.json`, and copies the manifest into `.claude-plugin/`). This keeps `VERSION`, `config.toml`, `plugin.json`, and `.savvy-manifest.json` in lockstep so `/sf:upgrade` has accurate hashes. Skip silently in scaffolded projects (no `VERSION`/`gen-manifest.sh`).
