...
- User runs `/intake "<idea text>"` (inline description).
- User runs `/intake --from-file <path>` (CLI-driven bootstrap).
...
6. **Batch 4 - Subagents.** Propose project-specific subagent definitions under `.claude/agents/` (e.g., `reviewer.md`, `test-writer.md`, stack-specific helpers). Each file has the standard subagent frontmatter. Commit `chore(intake): batch 4 - subagents`.
...
9. Print a final summary: files created per batch, commits made, deferred entries (if any). Suggest `/lint-framework` to verify and `/plan <first-spec>` to start work.
...
- Mid-batch interruption: leave uncommitted files staged but inform the user nothing was committed for that batch; resume cleanly on next `/intake`.
