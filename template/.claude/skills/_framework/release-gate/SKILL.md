...
7b. If this repo is the framework itself (a `VERSION` file and `scripts/gen-manifest.sh` exist at root), write the confirmed version into `VERSION` and `template/.claude/config.toml` (they must match), then run `scripts/build-plugin.sh`. Since authorship is inverted, the root payload (`commands/`, `skills/`, `hooks/`, `agents/`) is the source: the script stamps `plugin.json` from `VERSION` (aborting on a `VERSION`/`config.toml` mismatch), regenerates `.savvy-manifest.json`, and reverse-generates the legacy in-tree engine under `template/.claude/` from the root payload. This keeps `VERSION`, `config.toml`, `plugin.json`, and `.savvy-manifest.json` in lockstep so `/sf:upgrade` (legacy) has accurate hashes. Skip silently in scaffolded projects (no `VERSION`/`gen-manifest.sh`).
