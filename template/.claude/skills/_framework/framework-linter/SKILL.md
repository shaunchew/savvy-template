...
description: On-demand checker run via /lint-framework that emits a structured Markdown report of drift, missing files, malformed specs, budget violations, version drift, and stale _legacy/scratchpad folders.
---

# Framework Linter

On-demand diagnostic run via `/lint-framework`. Walks the repository, checks framework invariants, and emits a Markdown report grouped by severity. Performs no edits.

## When to invoke

- User runs `/lint-framework`.
...
6. **Stale `_legacy/` folders.** For each `_legacy/<migration>/`:
   - Folder mtime > 90 days AND no `REVIEW-LOG.md` = `warning`. Suggest `/legacy-review <migration>`.
