...
6. Create `ARCHIVE-REASON.md` in the spec folder containing: date, original status before archiving, the `<reason>` text, and a `Source: /spec-archive` line.
...
8. Run `/refresh-roadmap` to rewrite `ROADMAP.md` so the archived spec drops off Active.
