...
Push the current `ROADMAP.md` state and per-spec statuses to any enabled external mirror. Operates over the roadmap rather than per-spec tasks (the per-task sync is `/sync-notion`).
