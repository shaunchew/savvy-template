...
2. Read `ROADMAP.md` and parse the Active section into structured rows: `{category, NNN, name, status, summary}`. Also collect frontmatter `status:` from each linked `specs/<category>/<NNN>-*/sf:spec.md` and reconcile any mismatch against the roadmap line (prefer spec.md as ground truth; warn on drift).
