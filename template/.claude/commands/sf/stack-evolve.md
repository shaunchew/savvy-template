...
4. Draft two outputs in memory without writing yet:
   ...
   - **Deferred AGENTS.md additions:** the exact lines that would go under `## Stack` and/or `## Commands`, formatted as a pending-changes entry (timestamp, target `AGENTS.md`, field, proposed content, source `/sf:stack-evolve "<change>"`).
...
7. Print: "Wrote docs/decisions/<NNN>-<slug>.md. Queued 1 entry in .claude/pending-changes.md. Run /sf:curate after /sf:ship to apply AGENTS.md updates."
